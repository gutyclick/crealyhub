...
export type GeneratedImage = { bytes:Buffer; mimeType:"image/jpeg"; model:string; inputTokens:number; outputTokens:number };
export interface ImageProvider { generate(prompt:VisualPrompt,format:ImageFormat):Promise<GeneratedImage>; edit(source:Buffer,prompt:VisualPrompt,format:ImageFormat):Promise<GeneratedImage> }

const sizes:Record<ImageFormat,string>={POST:"1024x1280",CAROUSEL:"1024x1280",STORY:"1024x1824"};
function renderPrompt(spec:VisualPrompt,format:ImageFormat){return [`Instagram ${format} creative.`, `Subject: ${spec.subject}.`,`Composition: ${spec.composition}.`,`Art direction: ${spec.artDirection}.`,`Palette: ${spec.palette.join(", ")}.`,`Typography guidance: ${spec.typographyGuidance}.`,`Avoid: ${spec.negativeInstructions.join(", ")}.`,`No logos, watermarks, UI chrome, gibberish text, or unsafe edge placement.`].join("\n")}
